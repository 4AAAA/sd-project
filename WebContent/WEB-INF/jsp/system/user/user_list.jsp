					...
					<div class="row">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<c:if test="${QX.cha == 1 }">
								<td style="vertical-align:top;padding-left:2px;"><a class="btn btn-default btn-sm" onclick="searchs();"  title="检索">查询</a></td>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:10px;">
							<thead>
								<tr>
									...
									<th class="center">账号管理</th>
									<th class="center">客户名称</th>
									<!-- <th class="center">结算管理</th> -->
									<th class="center">用户类型</th>
									<th class="center"><i class="ace-icon fa fa-envelope-o"></i>用户邮箱</th>
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										...
										<tr>
											...
											<td class="center">
												...
												<!-- <a onclick="customer('${user.USERNAME}')" style="cursor:pointer;">[合同明细]</a> -->
											</td>
											<td class="center">${user.NAME }</td>
											<!-- 											<td class='center'>
												<a onclick="jingying('${user.USERNAME}')" style="cursor:pointer;">[经营状况]</a>
												<a onclick="kucun('${user.USERNAME}')" style="cursor:pointer;">[库存盘点]</a>
											</td> -->
					...
					<table style="width:100%;">
						<tr>
							<td style="vertical-align:top;">
								<c:if test="${QX.add == 1 }">
								<a class="btn btn-primary btn-sm" onclick="add();">新增用户</a>
								</c:if>
...
function customer(USERNAME){
	 ...
	 diag.Drag=true;
	 diag.Title ="合同信息";
	 diag.URL = '<%=basePath%>allcustomer/list.do?keywords='+USERNAME;
	 diag.Width = 800;
	 diag.Height = 600;
...
//经营状况
function jingying(USERNAME){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="经营状况";
	 diag.URL = '<%=basePath%>/login_default.do?USERNAME='+USERNAME;
	 diag.Width = 900;
	 diag.Height = 600;
	 diag.Modal = false;			//有无遮罩窗口
	 diag. ShowMaxButton = true;	//最大化按钮
...
//库存盘点
function kucun(USERNAME){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="库存盘点";
	 diag.URL = '<%=basePath%>allcustomer/kucunchar.do?USERNAME='+USERNAME;
	 diag.Width = 800;
	 diag.Height = 600;
	 diag.Modal = false;			//有无遮罩窗口
	 diag. ShowMaxButton = true;	//最大化按钮
